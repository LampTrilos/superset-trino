...
5. In `Security` select `Allow data upload`
 ...
 ## Connect to Trino via a Client (like DBeaver)
  Create a DB Connection (Trino) as `jdbc:trino://localhost:5542`
...
1. Upload the CSV to a **directory** inside the `raw-data` bucket in MinIO. We use the directory /ais for this examle.
