<template>
  ...
  <div class="col-12 col-md-12 q-mb-md">
    <SelectEnum
      filled
      v-model="copiedItem.sychnotitaKeraias"
      :label="t('sychnotitaKeraias')"
      data-cy="item-sychnotitaKeraias"
      clearable
      :readonly="!editable"
      enum-end-point="item-enums/sychnotitaKeraias"
      :class="restrictions ? 'required' : ''"
      :rules="restrictions ? requiredRule() : null"
      hide-bottom-space
    />
  ...
  <div class="col-12 col-md-12 q-mb-md">
    <SelectEnum
      filled
      v-model="copiedItem.mpantaKeraias"
      :label="t('mpantaKeraias')"
      data-cy="item-mpantaKeraias"
      clearable
      :readonly="!editable"
      enum-end-point="item-enums/mpantaKeraias"
      :class="restrictions ? 'required' : ''"
      :rules="restrictions ? requiredRule() : null"
      hide-bottom-space
    />
...
import { Keraia } from 'src/services/models';
import InputDate from 'components/inputs/InputDate.vue';
import TableMetaptosiKeraia from "components/tables/items/TableMetaptosiKeraia.vue";
...
const copiedItem: Keraia = computed({
...
function onMetaptosi() {
  ...
  axios
    .get('/api/v1/items-metaptosi/keraia/dto/' + selected.id)
    .then((res) => {
      // Clear initial values from the item object
      Object.keys(copiedItem.value).forEach((key) => {
        console.log(key)
        if (['serialNumber', 'paratiriseis', 'katastash', 'idMetaptosis', 'sychnotitaKeraias', 'mpantaKeraias'].includes(key)) {
