<template>
  ...
  <q-dialog v-model="metaptosiDialog" persistent>
    <q-card style="min-width: 80vw; max-width: 80vw">
      <q-card-section>
        <TableMetaptosiParelkomenaPompodekti
  ...
  <div class="col-12 col-md-12 q-mb-md">
    <SelectEnum
      filled
      v-model="copiedItem.eidosParelkomenou"
      :label="t('eidosParelkomenou')"
      data-cy="item-eidosParelkomenou"
      clearable
      :readonly="!editable"
      enum-end-point="item-enums/eidosParelkomenou"
      :class="restrictions ? 'required' : ''"
      :rules="restrictions ? requiredRule() : null"
      hide-bottom-space
    />
...
import TableMetaptosiParelkomenaPompodekti from "components/tables/items/TableMetaptosiParelkomenaPompodekti.vue";
...
function onMetaptosi() {
  ...
  axios
    .get('/api/v1/items-metaptosi/parelkomenoPompodekti/dto/' + selected.id)
    .then((res) => {
