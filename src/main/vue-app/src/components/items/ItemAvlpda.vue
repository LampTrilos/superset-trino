<template>
    ...
    <div class="row q-col-gutter-sm">
      <div class="col-8">
        <q-input
          ...
          clearable
          :readonly="!editable"
          hint="Αν δεν είναι τοποθετημένο σε όχημα πρέπει να αναφέρετε το λόγο στις παρατηρήσεις"
          hide-bottom-space
        />
      </div>
      <div v-if="copiedItem.arithmosKykloforiasOchimatos" class="col-4">
        <q-input v-model="validOchima" readonly borderless style="max-width: 160px">
  ...
  <div class="col-12 col-md-12 q-mb-md">
    <q-checkbox
      left-label
      v-model="copiedItem.memoryCard"
      :label="t('memoryCard')"
      data-cy="item-idPompodekti"
  ...
  <div class="col-12 col-md-12 q-mb-md">
    <SelectEnumById
      :key="idPompodektiKey"
      filled
      v-model="copiedItem.idPompodekti"
  ...
  <div class="col-12 col-md-12 q-mb-md">
    <SelectEnumById
      :key="idAvlKey"
      filled
      v-model="copiedItem.idAvl"
      :label="'Σειριακός Αριθμός Avl στο οποίο είναι συνδεδεμένο'"
      data-cy="item-idAvl"
      clearable
      :readonly="!editable"
      :option-label="'serialNumber'"
      enum-end-point="items/avl"
      :query-params="'draft=false'"
    />
  </div>
  <div class="col-12 col-md-12 q-mb-md">
    <q-input
      ...
      :class="restrictions ? ((!copiedItem.arithmosKykloforiasOchimatos && !copiedItem.paratiriseis) ? 'required' : '') : ''"
      :rules="restrictions ? ((!copiedItem.arithmosKykloforiasOchimatos && !copiedItem.paratiriseis) ? requiredRule() : null) : null"
    ></q-input>
  </div>
...
import { Avl } from 'src/services/models';
...
import TableMetaptosiAvlPda from "components/tables/items/TableMetaptosiAvlPda.vue";
...
const copiedItem: AvlPda = computed({
  get: () => props.modelValue,
  set: (value) => emit('update:modelValue', value),
});

const idPompodektiKey = ref(0);
const idAvlKey = ref(0);
...
function onMetaptosi() {
  ...
  axios
    .get('/api/v1/items-metaptosi/avlpda/dto/' + selected.id)
    .then((res) => {
      // Clear initial values from the item object
      Object.keys(copiedItem.value).forEach((key) => {
        console.log(key)
        if (['serialNumber', 'paratiriseis', 'katastash', 'idMetaptosis', 'arithmosKykloforiasOchimatos', 'idPompodekti', 'memoryCard', 'idAvl'].includes(key)) {
